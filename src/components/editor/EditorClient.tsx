...
import { Plus, ChevronLeft, Download, Share2, Play, Wand2, Trash2, Save, X, Copy, Check, FileJson, FileText, Globe, Loader2 } from "lucide-react";
...
export function EditorClient({ presentation, user }: Props) {
  ...
  const [shareUrl, setShareUrl] = useState(presentation.publishedUrl ? (process.env.NEXT_PUBLIC_APP_URL || "") + "/view/" + presentation.publishedUrl : "");
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} onClick={(e) => e.stopPropagation()} className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-sm">
              ...
              <div className="space-y-2">
                <button onClick={() => { exportAs("html"); setShowExport(false); }} className="w-full flex items-center gap-3 p-3 rounded-lg border border-neutral-200 hover:bg-neutral-50 transition-colors text-left">
                  <Globe size={18} className="text-neutral-500"/><div><p className="text-[13px] font-medium text-neutral-900">HTML Presentation</p><p className="text-[11px] text-neutral-400">Standalone HTML file with all slides</p></div>
                </button>
                <button onClick={() => { exportAs("markdown"); setShowExport(false); }} className="w-full flex items-center gap-3 p-3 rounded-lg border border-neutral-200 hover:bg-neutral-50 transition-colors text-left">
                  <FileText size={18} className="text-neutral-500"/><div><p className="text-[13px] font-medium text-neutral-900">Markdown</p><p className="text-[11px] text-neutral-400">Portable markdown with speaker notes</p></div>
                </button>
                <button onClick={() => { exportAs("json"); setShowExport(false); }} className="w-full flex items-center gap-3 p-3 rounded-lg border border-neutral-200 hover:bg-neutral-50 transition-colors text-left">
                  <FileJson size={18} className="text-neutral-500"/><div><p className="text-[13px] font-medium text-neutral-900">JSON Data</p><p className="text-[11px] text-neutral-400">Raw slide data for integration</p></div>
                </button>
              </div>
            </motion.div>
          </motion.div>
