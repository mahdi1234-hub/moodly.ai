          ...
          {slides.map((slide, i) => (
            <motion.div key={slide.id || i} layout className="relative group">
              <div
                ...
                className={"rounded-lg border cursor-pointer overflow-hidden transition-all " + (i === currentIndex ? "border-neutral-900 shadow-md ring-1 ring-neutral-900" : "border-neutral-200 hover:border-neutral-300")}
              >
                <SlideThumbnail content={slide.content} index={i} />
              </div>
              <button onClick={() => deleteSlide(i)} className="absolute top-1 right-1 opacity-0 group-hover:opacity-100 p-1 rounded-md bg-red-500 text-white transition-opacity shadow-sm">
                <Trash2 size={10}/>
              </button>
            </motion.div>
          ))}
          <button onClick={addSlide} className="w-full aspect-[16/10] rounded-lg border-2 border-dashed border-neutral-300 flex items-center justify-center text-neutral-400 hover:text-neutral-600 hover:border-neutral-400 transition-colors">
            <Plus size={16}/>
          </button>
        </div>
